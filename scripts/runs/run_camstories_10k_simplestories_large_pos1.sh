#!/bin/bash
#SBATCH --job-name=camstories_large_pos1
...
python train.py config/camstories_10k_simplestories_large.py \
  --posenc_type=sinusoidal --posenc_scale=1.0 \
  --out_dir=out/camstories_10k_simplestories/large_pos1 \
  --wandb_log=True --wandb_run_name=camstories-large-pos1
