#!/bin/bash
#SBATCH --job-name=cs_10k_shadow_unnorm_offbyone_target
...
python train.py config/shadow_audio/cs_10k_shadow_offbyone.py \
    --wandb_log=True \
    --wandb_run_name=cs-10k-shadow-unnorm-offbyone-target \
    --out_dir=out/shadow_audio/cs_10k_shadow_unnorm_offbyone_target \
    --shadow_audio_residual=unnormalised_residual \
    --softmax_off_by_one=True \
    --shadow_auxiliary_loss=target
