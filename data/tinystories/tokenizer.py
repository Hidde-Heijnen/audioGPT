...
class Tokenizer:
  ...
  def initialize(self):
    ...
    if self.k:
      ...
      eos_token_id_str = str(self.tokenizer.eos_token_id)
      self.top_k_tokens.append(eos_token_id_str)
      self.vocab_size +=1
  ...
  def encoder(self, input, padding=False, max_length=256, truncation=False):
    ...
    if self.k:
      eos_token_id_str = str(self.tokenizer.eos_token_id)
      tokens = torch.tensor([self.top_k_tokens_dict.get(str(token.item()), self.top_k_tokens_dict[eos_token_id_str]) for token in tokens.view(-1)], device=self.device).view(tokens.shape)
